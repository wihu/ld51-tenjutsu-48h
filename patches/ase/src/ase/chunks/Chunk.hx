...
class Chunk implements Serializable {
  ...
  function get_size():Int {
    return ChunkHeader.STRUCT_BYTE_SIZE + getSizeWithoutHeader();
  }
  ...
  public static function fromBytes(bytes:Bytes):Chunk {
    ...
    var header:ChunkHeader = ChunkHeader.fromBytes(bi.read(ChunkHeader.STRUCT_BYTE_SIZE));
    var chunkBytes:Bytes = bi.read(header.size - ChunkHeader.STRUCT_BYTE_SIZE);
